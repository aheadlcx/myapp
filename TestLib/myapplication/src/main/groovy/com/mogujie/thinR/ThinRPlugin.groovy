    ...
    void apply(Project project) {
        ...
        project.afterEvaluate {
            ...
            project.android.applicationVariants.all { variant ->
                ContextProvider contextProvider = new ContextProvider(project, variant.name
                        .capitalize() as String, extension.buildVariants as String)
...
//                    throw new GradleException("unknown file input file ${file} ")
    ...
    void processDir(File dir, ThinRProcessor thinRProcessor) {
        dir.eachFileRecurse { file ->
            ...
            if (file.name.endsWith(".class")) {
                ...
                PrintUtil.verbose(" processDir " + "file--> " + file.absolutePath)
    ...
    void processJar(File jarFile, ThinRProcessor thinRProcessor) {
        ...
        while (je.hasMoreElements()) {
            ...
            PrintUtil.verbose(" processJar " + "jarEntry--> " + jarEntry.name)
            if (thinRProcessor.needKeepEntry(jarEntry.getName())) {
                jos.putNextEntry(zipEntry);
                jos.write(bytes);
                jos.closeEntry();
                PrintUtil.verbose(" processJar  needKeepEntry" + "jarEntry--> " + jarEntry.name)
            } else {
                PrintUtil.verbose(" processJar  needKeepEntry  not" + "jarEntry--> " + jarEntry.name)
            }
