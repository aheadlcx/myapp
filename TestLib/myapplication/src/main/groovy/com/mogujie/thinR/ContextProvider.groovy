...
class ContextProvider {
    ...
    public ContextProvider(Project project, String varNameCap, String buildVariants) {
        ...
        PrintUtil.verbose("r isHighVersion ==> " + isHighVersion + " -- varNameCap = " + varNameCap)
        isHighVersion = true
        if (isHighVersion) {
...
//            dexTask = project.tasks.findByName("transformClassesWithDexForRelease")
        ...
        if (dexTask == null) {
            throw new GradleException("Can not found dex task!" + "r isHighVersion ==> " + isHighVersion + " -- varNameCap = " + varNameCap)
        }
